...
## 追加実装確認用データ
- 出品者1では、取引中データを3件確認できます（腕時計は未読2件、HDDは未読1件、ノートPCは既読済み）
- 取引中の商品タブから取引画面へ遷移し、未読メッセージ件数のバッジ表示を確認できます
- 出品者2では、完了済み取引データを確認できます
- 完了済み取引には相互評価データを投入しているため、マイページで評価平均を確認できます
- 未使用ユーザーは何にも紐づいていない状態です

---

## 注意事項
- Laravel アプリケーション本体は `src` ディレクトリにあります。
- 固定のダミー画像（商品画像・アイコン画像など）は `src/public/` 配下に配置しています。
- ユーザーがアップロードする画像は `storage/app/public/` を利用します。
...
- 取引画面で入力中の本文は取引ごとに保持され、サイドバーから別取引へ移動して戻った場合でも復元されます
